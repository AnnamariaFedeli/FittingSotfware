...
def MAKE_THE_FIT(spec_e, spec_flux, e_err, flux_err, ax, direction='sun', which_fit='best', e_min=None, e_max=None, g1_guess=-2., g2_guess=None, alpha_guess=5., break_guess=0.065, cut_guess = 0.12, c1_guess=None, use_random = False, iterations = 10, path = None, path2 = None):
	...
	if which_fit == 'cut':
		...
		if use_random :
			for i in range(iterations):
				...
				cut_random = np.random.choice(cut_array, 1)[0]
				c1_random = np.random.choice(c1_array, 1)[0]
				result_cut_random = pl_fit.cut_break_pl_fit(x = spec_e, y = spec_flux, xerr = e_err, yerr = flux_err, gamma1=g1_random, gamma2=g2_random, c1=c1_random, alpha=alpha_random, E_break=break_random, E_cut = cut_random, print_report=False, maxit=10000)
				breakp_cut = result_cut_random.beta[4]
				cut = result_cut_random.beta[5]
				if breakp_cut < e_min or breakp_cut > e_max:
	...
	if which_fit_final == 'broken':
		...
		ax.axvline(x=breakp, color='blue', linestyle='--', label=r'$\mathregular{E_b=}$ '+str(round(breakp, ndigits=1))+'\n'+r"$\pm$"+str(round(breakp_err, ndigits=0))+' keV')
